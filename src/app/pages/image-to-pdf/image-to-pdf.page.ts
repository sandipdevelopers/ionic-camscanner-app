...
@Component({
  ...
  templateUrl: './image-to-pdf.page.html',
  styleUrls: ['./image-to-pdf.page.scss'],
})
export class ImageToPdfPage implements OnInit {
  imageUploded: any[] = []
  savLoader: any;
  nameformat = /[`!#$%^&*()+\=\[\]{};':"\\|,<>\/?~]/;
  printSize: any = 'A3';
  ...
  async saveAsPDF() {
    if (!this.imageUploded.length) {
      return;
    }

    let digit = await this.service.generateRandom5Digit();
      let defaultFile = `PhotoToPdf-${digit}`;
    ...
    modal.onDidDismiss().then((result: any) => {
      if (result.data) {
        ...
        this.showLoader().then(async () => {
          ...
          for (i in this.imageUploded) {
            if (i == '0') {
              data.push({
                image: this.imageUploded[i],
                fit: fit,
                alignment: 'center',
              });
            } else {
  ...
  swipeImagePrev(index: any) {
    if (index <= 0 || index >= this.imageUploded.length) {
      return;
    }

    const item = this.imageUploded[index];

    this.imageUploded.splice(index, 1);

    this.imageUploded.splice(index - 1, 0, item);
  }

  swipeImageNext(index: any) {
    if (index < 0 || index >= this.imageUploded.length - 1) {
      return;
    }
  ...
  async cropImage(index: any) {
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        imageSrc: this.imageUploded[index],
      },
    });

    modal.onDidDismiss().then((result: any) => {
      if (result.data) {
        this.imageUploded[index] = result.data;
      }
    });
  ...
  async uploadFromGallery() {
    const options: ImagePickerOptions = {
      quality: 100,
      outputType: 1,
      maximumImagesCount: 50 - this.imageUploded.length,
    };
